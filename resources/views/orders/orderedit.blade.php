                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="supplier_id" id="supplier_id" disabled>
                            @foreach($suppliers as $supplier)
                                <option value="{{ $supplier->id }}" data-valoriva={{$supplier->valor}} data-iva={{$supplier->tipo}}>{{ $supplier->company}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        @foreach ($orders as $order)
                            @if ($order->status == 'En proceso')
                                <select class="form-control" name="require_employee_id" id="require_employee_id" required>
                            ...
                            @else
                                <input type="require_employee_id" class="form-control" id="require_employee_id" name="require_employee_id" value="{{$order->name}}" readonly>
                            @endif
                        @endforeach
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-3">
                    <div class="form-group">
                        ...
                        @foreach($suppliers as $supplier)
                            <input type="text" class="form-control" id="iva" name="iva" value={{$supplier->tipo}} readonly >
                        @endforeach
                    </div>
                </div>
                <div class="col-9">
                    <div class="form-group">
                        ...
                        @foreach ($orders as $order)
                            ...
                            @else
                                <input type="description" class="form-control" id="description" name="description" value="{{$order->description}}" readonly>
                            @endif
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        <label for="orderstatus_id">Status:</label>
                            @foreach ($orderstatus as $status)
                                @if ($status->name == 'En proceso')
                                    <select class="form-control" name="orderstatus_id" id="orderstatus_id" style="width: 250px;" required>
                                        <option value="1">En proceso</option>
                                        <option value="2">Comprada</option>
                                        <option value="3">Cancelada</option>
                                    </select>
                                @elseif($status->name == 'Comprada')
                                    <select class="form-control" name="orderstatus_id" id="orderstatus_id" style="width: 250px;" disabled>
                                        <option value="2">Comprada</option>
                                    </select>
                                @elseif($status->name == 'Cancelada')
                                    <select class="form-control" name="orderstatus_id" id="orderstatus_id" style="width: 250px;" disabled>
                                        <option value="3">Cancelada</option>
                                    </select>
                                @endif
    ...
    <div class="card">
        <div class="card-body">
            <div class="row">
                @foreach ($orderstatus as $status)
                    @if ($status->name == 'En proceso')
                        <select class="form-control" name="product_id" id="product_id" style="width: 250px;" required>
                    ...
                    @else
                        <select class="form-control" name="product_id" id="product_id" style="width: 250px;" disabled>
                            <option value="" disabled selected>-- Seleccione un producto--</option>
                        </select>
                        <button id="add" class="btn btn-success btn sm" data-toggle="modal" data-target="#editModal" disabled>
                            <i class="fa fa-plus"></i>
                        </button>
                    @endif
                @endforeach
...
<script>
    document.addEventListener("DOMContentLoaded", function(event){
        //id de la orden obtenida desde la url
        var ordid = location.pathname.split('/')[2];

        //valor iva proveedor
        let valorIVA = $('#supplier_id').find(':selected').data('valoriva')
            
        let productsDetailDelete = [];
        let productsDetail = {!! json_encode($order_details) !!};
        ...
        $('#add').on('click', function(){  
            ...
            //estado de la orden
            if ($('#orderstatus_id').find(':selected').text() == 'En proceso'){
                $.cargarDetallesTabla(productsDetail.filter(x => x.eliminar == 0), false);
                ;
            }else{
                $.cargarDetallesTabla(productsDetail.filter(x => x.eliminar == 0), true);
            }
        ...
        $.cargarDetallesTabla = function(data, disabled){
            ...
            const ButtonContent = disabled == true ? '<button type="button" disabled="disabled" class="btn btn-danger eliminarDetalle"> <i class="fas fa-minus"></i> </button>' : '<button type="button" class="btn btn-danger eliminarDetalle"> <i class="fas fa-minus"></i> </button>';
            ...
            $('#tblDetails tbody button.eliminarDetalle').click(function (e) {
                ...
                if (DatosTabla != undefined) {
                    ...
                    if (FilaSeleccionada != undefined && FilaSeleccionada != null) {
                        ...
                        //     FilaSeleccionada.eliminar = 1;
                        //     productsDetailDelete.push(FilaSeleccionada)
                        // }
                        ...
                        //     //return data.GUID != FilaSeleccionada.GUID;
                        //     return data.eliminar == false;
                        // });
            ...
            axios.post('/order/'+ordid+'/update', {
                data: JSON.parse(
                    JSON.stringify({
                        ...
                        total: productsDetail.filter(x => x.eliminar == 0).map(x => x.product_total).reduce(function(a, b){return (+a) + (+b);}),
                        details: productsDetail,
                        detailsDelete: productsDetailDelete
                    })
                )
            })
        ...
        if ($('#orderstatus_id').find(':selected').text() == 'En proceso'){
            ...
            $.cargarDetallesTabla(productsDetail, true);
        }
        //$.cargarDetallesTabla(productsDetail, true);

        //DEVUELVE UN GUID UNICO 
        $.fnGuid = function () {
        function s4() {
